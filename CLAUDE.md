...
Pre-launch smoke tests: `python tests/smoke_test.py` (79 offline unit tests, no DB required). No linter configured. Logging goes to `quant.log` and stdout. Set `logging.DEBUG` in main.py to see signal rejection details.
...
- **Mean reversion / contrarian**: 180-point long cache (~30 min). Detects >8% moves, blocks >30% moves (news-driven). Volume filter: >2.0x = skip (informed money). Smooth vol confidence scaling: low vol → high conf (capped 0.95), high vol → low conf (floor 0.1). `is_contrarian` threshold 0.5. EWMA reversion target, confidence-weighted shift.
- **NegRisk arbitrage**: Groups markets by negRiskMarketID, normalizes prices to sum=1.0.
- **Kelly criterion**: 0.15 fraction (conservative), guarded against extreme odds (b < 0.01 or b > 100 → 0), spread penalty (3-10¢ → 1.0-0.3x multiplier, clamped to [0.3, 1.0]), capped at MAX_KELLY_FRAC of bankroll. **Uncertainty scaling**: `kelly × (0.5 + 0.5 × n_sources/7)` — 1 evidence source → Kelly ×0.57, all 7 → Kelly ×1.0. Contrarian trades: Kelly × 0.5. Per-theme Bayesian Kelly multiplier (0.3–2.0×) from trade history. Portfolio correlation penalty (see below).
...
- **Staged SL with grace period**: First 2 hours: emergency SL only (1.5× default, e.g., 37.5% at SL=25%). After 2h: normal SL. Data: <1h=32% WR, 1-3h=49% WR, 3h+=63% WR — noise kills early positions. Vol SL disabled — fixed SL from config (data showed SL=0.25-0.30 beats all vol-adjusted SLs: 8%→24% WR, 15%→34%, 20%→56%, 25%→75%+).
...
- **Conservative Kelly**: 0.15 fraction of full Kelly. Contrarian trades halved again. Zero stake on negative bankroll. **Bimodal sizing**: $5-10 stake range is toxic (48.6% WR, -$57 total) — stakes in this range pushed to $4 (low Kelly) or $12 (Kelly ≥8%).
...
- Pre-launch smoke tests (79 offline unit tests + 7 online checks) — bot refuses to start if any fail
