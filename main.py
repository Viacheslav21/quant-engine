...
_SAFE_CONFIG_KEYS = {
    ...
    "MAX_MARKET_DAYS", "MIN_VOLUME", "USE_PROSPECT",
...
async def execute_signal(signal: dict, db: Database, telegram: TelegramBot, config: dict,
    ...
    # Blocked themes — managed via dashboard (patterns.blocked = TRUE)
    theme = signal.get("theme", "other")
    blocked_themes = await db.get_blocked_themes()
...
async def main():
    ...
    await db._cleanup_stale_config(["SKIP_SPORTS", "SL_PCT"])
    ...
    try:
        ...
        _smoke_path = _pathlib.Path(__file__).parent / "tests" / "smoke_test.py"
        result = subprocess.run([_sys.executable, str(_smoke_path)],
